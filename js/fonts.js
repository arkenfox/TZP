...
function set_fntList() {
	...
	let fntKey = ['android','linux','mac','windows']
	for (let i=0; i < fntMaster.length; i++) {
		let array = fntMaster[i]
		array.sort()
		array = array.filter(function(item, position) {return array.indexOf(item) === position})
		fntLists.push(array)
		let str = "fonts_fonts_"+ fntKey[i] + "_list_skip"
		if (fntKey[i] == isOS) {
			fntList = array
			fontslists += buildButton("12", str, array.length +" "+ fntKey[i])
		} else {
...
function get_unicode() {
	...
	return new Promise(resolve => {
		...
		function run() {
			...
			if (!fntPrimed) {
				fntPrimed = true
				log_perf("time since unicode priming",fntStart,"ignore")
			}
...
function outputFonts() {
	...
	Promise.all([
		...
		results.forEach(function(currentResult) {
			...
			dom.fontslists.innerHTML = fontslists
		})
...
function prime_unicode() {
	// FF86+ 1676966: gfx.font_rendering.fallback.async
		// when chars that trigger the global fallback search (iterating over all available fonts to try and
