...
function get_fonts() {
	return new Promise(resolve => {
		...
		function finish_up() {
			// unhide, cleanup details, perf, resolve
		...
		getFonts().then(res => {
			...
			if (res == "n/a") {
				...
				dom.fontFound.innerHTML = (fontCount ? fontsFound.join(", ") : "no fonts detected")
				// set return
				if (greatest.count > 3) {
...
function get_unicode() {
	...
	return new Promise(resolve => {
		...
		function output() {
			...
			dom.ug2.innerHTML = sha1(res.join()) + (isCanvas ? "" : sb +"[canvas]"+ sc)
